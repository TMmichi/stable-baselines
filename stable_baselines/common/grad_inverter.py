...
class grad_inverter:
    def __init__(self, action_bounds):
        ...
        self.min_bound_compare = tf.where(
            tf.greater(self.pmin, self.action_input),
            self.pmin - self.action_input,
            self.act_grad)
        self.max_bound_compare = tf.where(
            tf.greater(self.action_input, self.pmax), 
            self.pmax - self.action_input,
            self.act_grad)
        self.grad_inverter = tf.where(
            tf.greater(self.act_grad, self.zeros_act_grad_filter),
            self.max_bound_compare, 
            self.min_bound_compare)

...
from stable_baselines.common.policies import ActorCriticPolicy, RecurrentActorCriticPolicy, HierarchicalPolicy
from stable_baselines.common.schedules import get_schedule_fn
from stable_baselines.common.tf_util import total_episode_reward_logger
from stable_baselines.common.math_util import safe_mean

class PPO2_MULTI(ActorCriticRLModel):
    """
    ...
    def setup_custom_model(self, primitives, load_value=True):
        with SetVerbosity(self.verbose):
            assert issubclass(self.policy, HierarchicalPolicy), "Error: the input policy for the PPO2 model must be " \
                                                               "an instance of common.policies.HierarchicalPolicy."
            self.n_batch = self.n_envs * self.n_steps
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    with tf.variable_scope('model/pi/train'):
                        self.params = tf.trainable_variables()
                        if self.full_tensorboard_log:
                            for var in self.params:
                                tf.summary.histogram(var.name, var)

...
from stable_baselines.cql.policies import CQLPolicy
from stable_baselines import logger


class CQL(OffPolicyRLModel):
    """
    CONSERVATIVE Q-LEARNING FOR OFFLINE RL (CQL)

    This implementation borrows code from original implementation (https://github.com/aviralkumar2907/CQL)
    from OpenAI Spinning Up (https://github.com/openai/spinningup)
    Paper: https://arxiv.org/abs/2006.04779

    :param policy: (CQLPolicy or str) The policy model to use (MlpPolicy, CnnPolicy, LnMlpPolicy, ...)
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=1e-3, buffer_size=50000,
                 ...
                 tau=1e-2, ent_coef='auto', target_update_interval=1,
                 policy_eval_start=0, num_qs=2, min_q_version=3, temp=1.0, min_q_weight=1.0,
                 max_q_backup=False, deterministic_backup=True, num_random=10, with_lagrange=False, lagrange_thresh=0.0,
                 gradient_steps=1, target_entropy='auto', action_noise=None,
                 random_exploration=0.01, verbose=0, tensorboard_log=None,
                 _init_setup_model=True, policy_kwargs=None, full_tensorboard_log=False,
                 seed=None, n_cpu_tf_sess=None):

        super(CQL, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose,
                                  policy_base=CQLPolicy, requires_vec_env=False, policy_kwargs=policy_kwargs,
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("model", reuse=False):
                    ...
                    if self.with_lagrange:
                        self.log_ent_coef_prime = tf.zeros(1)
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    qf1_loss = 0.5 * tf.reduce_mean((q_target - qf1) ** 2)
                    qf2_loss = 0.5 * tf.reduce_mean((q_target - qf2) ** 2)
                    ...
                    if not isinstance(self.ent_coef, float):
                        ...
                        if self.with_lagrange:
                            self.ent_coef_prime_optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate_ph)
                    ...
                    with tf.control_dependencies([policy_train_op]):
                        ...
                        if ent_coef_loss is not None:
                            with tf.control_dependencies([train_values_op]):
                                ...
                                if self.with_lagrange:
                                    self.ent_coef_prime_optimizer.minimize(var_list=self.log_ent_coef_prime)
    ...
    def learn(self, total_timesteps, callback=None,
              log_interval=4, tb_log_name="CQL", reset_num_timesteps=True, replay_wrapper=None):
    ...
    def action_probability(self, observation, state=None, mask=None, actions=None, logp=False):
        if actions is not None:
            raise ValueError("Error: CQL does not have action probabilities.")

        warnings.warn("Even though CQL has a Gaussian policy, it cannot return a distribution as it "
